...
void UIPEthernetClass::tick(void)
{
	...
	int count = network.getWaitingPacketCount();
 ...
 */
        bool UIPEthernetClass::network_send(void) {
            ...
            {
#ifdef UIPETHERNET_DEBUG
                //printf("Enc28J60Network_send uip_packet: %s, hdrlen: %i\n", uip_packet, uip_hdrlen);
printf("Enc28J60Network_send uip_packet: _, hdrlen: %i\n", uip_hdrlen);
#endif
            ...
            {
#ifdef UIPETHERNET_DEBUG
                //printf("Enc28J60Network_send uip_buf (uip_len): %d, packet: %s\n", uip_len, uip_packet);
                printf("Enc28J60Network_send uip_buf (uip_len): %d, packet: _\n", uip_len);
#endif
 ...
 */
...
#endif
        {
            ...
            printf("chksum uip_buf[%d-%d]: %x\n",
		UIP_IPH_LEN + UIP_LLH_LEN,
...
#ifdef UIPETHERNET_DEBUG_CHKSUM
                printf("chksum uip_packet(%s)[%d-%d]: %x,
			uip_packet,
			UIP_IPH_LEN + UIP_LLH_LEN + upper_layer_memlen,
			UIP_IPH_LEN + UIP_LLH_LEN + upper_layer_len,
			htons(sum)
		);
#endif
            }
